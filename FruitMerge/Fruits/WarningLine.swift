...
class WarningLine: SKShapeNode {

    init(in container: FruitContainerShape) {
        super.init()

        let startPoint = CGPoint(x: 0, y: container.containerSize.height * 0.8)
        let endPoint = CGPoint(
            x: container.containerSize.width,
            y: container.containerSize.height * 0.8)
    ...
    func blink() {
        // If already blinking, don't start another blink animation.
        if self.action(forKey: "blinkWarningLine") != nil {
            return
        }
        ...
        self.run(fullSequence, withKey: "blinkWarningLine")
    }
